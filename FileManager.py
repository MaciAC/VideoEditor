...
class FileManager:
    ...
    def _get_filepaths(self, folder_name):
        return [join(self.base_folder, folder_name, filename) for filename in
                    listdir(join(self.base_folder, folder_name))]
    ...
    def create_normalized_audiofiles(self) -> list[str]:
        ...
        if not exists(out_folder):
            mkdir(out_folder)
        out_paths = []
        for input_video_path in tqdm(self.video_filepaths + [self.audio_filepath], total=len(self.video_filepaths)):
            out_path = join(self.base_folder,
                            NORM_AUDIO_FOLDER,
    ...
    def convert_video_for_instagram(self, input_video_path, output_video_path):
        ffmpeg_command = [
            "ffmpeg",
            "-i", input_video_path,
            "-vf", "scale=1080:1080:force_original_aspect_ratio=increase,crop=1080:1080",  # Resize and crop
            "-c:v", "libx264",  # Video codec (H.264)
            "-preset", "slow",  # Choose encoding speed (options: ultrafast, superfast, veryfast, faster, fast, medium, slow, slower, veryslow)
    ...
    def cut_video(self, input_video_path, output_video_path, start_offset_seconds, duration):
        ffmpeg_command = [
            "ffmpeg",
            "-ss", str(start_offset_seconds),  # Set the start offset in seconds
            "-t", str(duration),
            "-i", input_video_path,
    ...
    def cut_videos_based_on_offsets(self):
        ...
        audio_out_path = join(self.base_folder,
                    VIDEO_SYNC_FOLDER,
                    self.audio_filepath.split('/')[-1].rsplit('.', 1)[0] + '.wav'
                    )
        audio_duration = self.cut_audio(self.audio_filepath, audio_out_path, -min_offset if min_offset < 0 else 0.0)
        videos_out_path = []
        for video_path, offset in tqdm(zip(self.video_filepaths, self.offsets), total=len(self.offsets)):
            out_path = join(self.base_folder,
                        VIDEO_SYNC_FOLDER,
