...
class FFmpegWrapper:
    FFMPEG_COMMAND = 'ffmpeg -loglevel error -i "{i_path}" {parameters} "{o_path}" {force}'
    ...
    def cut_audio(self, i_path:str, o_path:str, start_offset_seconds:int, duration:int) -> str:
        self.i_path = i_path
        self.o_path = o_path
        parameters = [
            "-ss",
    ...
    def get_audio_duration(self, i_path):
        ...
        while retry:
            try:
                ...
                result = subprocess.run(self.FFMPEG_COMMAND_INSPECT, capture_output=True, text=True)
                # Extract the duration from FFmpeg output using regex
                duration_match = re.findall(r"Duration: \d+:\d+:\d+\.\d+", result.stderr)
